...
Browser-backed availability fetching is isolated to `recreation.py`, `reserve_california.py`, and the Chrome/Playwright layer; Gmail's optional OAuth authorization flow may also open a local browser. Outdoorithm, optional Ollama analysis, and Gmail delivery are separate network integrations. Core filtering, aggregation, and state logic operates on `Campsite`/`Match`/`MatchWindow` models as plain Python, with provider, AI, and email clients mocked in unit tests.
...
3. `main.group_searches_by_campground` groups searches by campground URL, using the union of their date ranges (and maximum stay length) for one fetch even when individual windows differ. `main.interleave_search_groups_by_domain` then walks the provider-domain queues round-robin, alternating domains while more than one has pending work.
...
6. `results.aggregate_match_windows` collapses raw matches into `MatchWindow`s: one representative site per campground/check-in date/stay length, with consecutive check-in dates for the same campground and stay length merged into a single window so a long run of open dates does not flood the report.
...
- `--reservecalifornia-locks`: with `--park-url`, fetches raw ReserveCalifornia lock-icon slices for one date range and prints them or writes `--locks-output`; this one-off path does not call `find_locked_matches`. Without `--park-url`, it runs the `locked_searches` entries from `config/searches.yaml`, calls `find_locked_matches`, aggregates the results, and writes `data/locked-matches.json`/`.csv` with the same optional AI/email pipeline as a normal scan.
...
- `fetch_campsites_for_search(...)` provides a common dispatcher and returns a `list[Campsite]`, while forwarding only the options each concrete provider supports. Recreation.gov receives request-delay and retry settings; ReserveCalifornia receives request delay; Outdoorithm receives request delay, retries, raw-data capture, and the retry logger.
...
`availability.py` retains Recreation.gov URL and payload parsing helpers, while its `find_matches` function is provider-agnostic and applies date-window/site filters to any provider's `list[Campsite]`. A new provider owns its payload parser and returns `Campsite` models through the dispatcher; shared matching tests can then exercise `find_matches` without mocking a browser.
...
`availability.py`, `results.py`, `cache.py`, `state.py`, `ai.py`, `alerts.py`, `gmail.py` (with mocked Google clients), and the parsing/config helpers in `providers.py`/`config.py`/`outdoorithm.py` are unit-testable without Chrome or real network access; see `tests/`. Keeping `recreation.py`/`reserve_california.py`'s browser-driving code thin and separating provider parsing from shared matching, aggregation, and state logic is intentional, so selector or upstream API drift can be fixed without disturbing the core search rules.
