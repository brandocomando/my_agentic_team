...
The search calendar is set to **one day before `START_DATE`, for one night** so unreleased arrival dates do not need to be selected in the calendar. During the release window, the helper reloads the whole page before each availability check, restores the previous-day one-night search if needed, watches the requested site's actual `START_DATE` cell, then selects `NIGHTS` in the site's reservation-duration dropdown before clicking Book Now. If the requested duration is unavailable, it stops without booking. The reservation details must still match the actual arrival date and full requested stay.

By default this waits one second between attempts, plus page-load and date-setup time, from 7:59:30 AM to 8:01:00 AM America/Los_Angeles, clicks the matching site cell for `START_DATE` if it becomes selectable, clicks the enabled `Book Now` button, acknowledges an `OK` alert popup if one appears, validates that the reservation details page shows the requested arrival date and `NIGHTS`, fills pre-cart details, accepts terms, clicks `Reserve Unit`, then clicks `Go To Checkout` if it appears. If checkout address fields are configured, it fills those too. It does not complete payment or enter card details. Override the window with:
